export default class ComponenteInstalacion {
    ...
    getCostoUbicacion(pantalla) {
        ...
        if (pantalla.carasVV) {
            Object.values(pantalla.carasVV).forEach(cara => {
                costoTotal += Number(cara.costoInstalacion) || 0;
            });
        } else if (pantalla.capex) {
            costoTotal = Number(pantalla.capex.costoInstalacion) || 0;
        }
        return costoTotal;
    }
    ...
    getTotales(pantallas) {
        ...
        let totalInstalaciones = 0, costoTotal = 0;
    ...
    renderFicha(totales) {
        const renderFechas = (data, colorHex, labelVacio) => {
            ...
            return data.map((item, idx) => {
                ...
                <div class="ficha-filter-row"
                     onclick="app.filtrarPorAtributo('costoInstalacion', 'fechaInstalacionPantalla', '${valorFiltro}', this)"
        ...
        const estadoColor = totales.sinDatos > 0 ? '#f59e0b' : '#10b981';

        return `
            <div style="font-size:0.7rem; color:#64748b; font-weight:800; text-transform:uppercase; letter-spacing:1px;">🔌 COMPONENTE</div>
            <div style="font-size:1.6rem; font-weight:900; color:#0f172a; margin-bottom:25px;">INSTALACIÓN ELÉCTRICA</div>
            
            <div style="display:flex; gap:12px; margin-bottom:25px;">
                <div class="ficha-filter-card" onclick="app.filtrarPorAtributo('costoInstalacion', 'global', 'todas', this)" 
                    ...
                    <div style="font-size:2rem; font-weight:900; color:#0f172a; margin-top:10px;">${totales.totalInstalaciones}</div>
                ...
                <div class="ficha-filter-card"
                    ...
                    <div style="font-size:0.65rem; color:#64748b; font-weight:800;">✅ Con datos</div>
                    <div style="font-size:2rem; font-weight:900; color:#10b981; margin-top:10px;">${totales.conDatos}</div>
                </div>
                <div class="ficha-filter-card"
                     style="flex:1; border:1px solid #e2e8f0; border-radius:12px; padding:18px 10px; text-align:center; background:#fff;">
            ...
            <div style="border:1px solid #e2e8f0; border-radius:12px; padding:15px; margin-bottom:15px; background:#fff;">
                ...
                <div class="ficha-filter-row" style="display:flex; justify-content:space-between; padding:14px 12px; border-bottom:1px solid #f1f5f9;">
                    <div><div style="font-weight:900;">Con datos técnicos</div></div>
                    <div style="font-weight:900; color:#10b981; font-size:1.3rem;">${totales.conDatos}</div>
                </div>
                <div class="ficha-filter-row" style="display:flex; justify-content:space-between; padding:14px 12px;">
                    <div><div style="font-weight:900;">Sin datos técnicos</div></div>
            ...
            <div style="border:1px solid #e2e8f0; border-radius:12px; padding:15px; margin-bottom:15px; background:#fff;">
                ...
                <div style="display:flex; justify-content:space-between; padding:14px 12px; border-bottom:none;">
                    <div><div style="font-weight:900;">Promedio por Instalación</div></div>
                    <div style="font-weight:900; color:#6366f1;">${totales.totalInstalaciones > 0 ? this.formatMoney(totales.costoTotal / totales.totalInstalaciones) : '$0'}</div>
                </div>
            </div>

            <div style="background:#eff6ff; border:1px solid #bfdbfe; border-radius:10px; padding:12px 15px; margin-top:15px;">
                <div style="font-size:0.7rem; color:#1e40af; font-weight:700;">💡 1 instalación eléctrica por columna. Incluye medidor CFE, interruptor 3x100A y 8 interruptores 2x20A.</div>
            </div>`;
    }
    ...
    renderDetalleUbicacion(pantalla) {
        ...
        let html = `
            <div style="margin-bottom:15px;">
                <div style="display:flex; gap:10px; margin-bottom:10px;">
                    <div style="flex:1; height:120px; background:#e2e8f0; border-radius:8px; overflow:hidden; border:1px solid #e2e8f0;">
                        <div id="mapa-det-inst-${pantalla.id}" style="width:100%; height:100%;"></div>
                    </div>
            ...
            <div style="background:#f0fdf4; border:1px solid #bbf7d0; border-radius:10px; padding:12px; margin-bottom:15px;">
                ...
                <div style="font-size:0.7rem; color:#166534;">1 Instalación eléctrica para toda la columna</div>
            </div>`;
        ...
        caras.forEach(cara => {
            ...
            const tieneDatos = fechaInst !== 'N/D';
            let costoBase = 0;
            if (pantalla.carasVV && pantalla.carasVV[cara]) {
                costoBase = Number(pantalla.carasVV[cara].costoInstalacion) || 0;
            }
            ...
            <div style="background:#fff; border:1px solid #e2e8f0; border-radius:10px; padding:15px; margin-bottom:10px;">
                <div style="display:flex; justify-content:space-between; border-bottom:1px solid #f1f5f9; padding-bottom:8px; margin-bottom:10px;">
                    ...
                    <span style="background:${tieneDatos ? '#f0fdf4' : '#fef3c7'}; color:${tieneDatos ? '#10b981' : '#f59e0b'}; padding:3px 10px; border-radius:6px; font-size:0.65rem; font-weight:800;">${tieneDatos ? 'CON DATOS' : 'SIN DATOS'}</span>
                </div>
        ...
        html += `
            <button onclick="app.abrirModalRadiografia('costoInstalacion', '${pantalla.id}')" style="width:100%; padding:14px; background:#0f172a; color:#fff; border:none; border-radius:10px; font-weight:800; font-size:0.85rem; cursor:pointer; text-transform:uppercase; letter-spacing:1px; margin-top:15px;">Radiografia de la Ubicacion</button>`;
