export default class ComponenteGasolina {
    ...
    _obtenerValor(gasto) {
        ...
        for (const clave of Object.keys(gasto)) {
            ...
            if (claveLower.includes('gasolina') && claveLower.includes('costo')) {
                const val = Number(gasto[clave]) || 0;
                if (val > 0) return val;
            }
        }
    ...
    getTotales(pantallas, meses) {
        ...
        pantallas.forEach(p => {
            // ✅ Usar el método unificado para calcular el costo
    ...
    renderFicha(totales) {
        const mesesHTML = Object.keys(totales.gastosPorMes).map(mes => `
            <div class="ficha-filter-row" onclick="app.filtrarPorAtributo('gasolina', 'mes', '${mes}', this)" 
        ...
        return `
            ...
            <div style="font-size:1.6rem; font-weight:900; color:#0f172a; margin-bottom:25px; letter-spacing:-0.5px;">GASOLINA</div>
            
            <div style="display:flex; gap:12px; margin-bottom:25px;">
                <div class="ficha-filter-card" onclick="app.filtrarPorAtributo('gasolina', 'global', 'todas', this)" 
            ...
            <div style="border:1px solid #e2e8f0; border-radius:12px; padding:15px; background:#fff;">
                ...
                <div class="ficha-filter-row" onclick="app.filtrarPorAtributo('gasolina', 'global', 'todas', this)" 
        ...
        return `
            <div style="background:#fff; border:1px solid #e2e8f0; border-radius:10px; padding:15px; margin-bottom:15px;">
                <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #f1f5f9; padding-bottom:10px; margin-bottom:12px;">
                    <span style="font-weight:900; color:#0f172a;">⛽ Gasolina - ${pantalla.id}</span>
                ...
                <div style="background:#f8fafc; padding:12px; border-radius:8px; text-align:center; margin-bottom:12px;">
                    <div style="font-size:0.7rem; color:#64748b;">Gasto Total Gasolina</div>
                ...
                ${gastosConValor.length > 0 ? `
                    ...
                    ${gastosConValor.map(g => {
                        ...
                        const litros = g.Litros || g.litros || 'N/D';
                        const dias = g['Días'] || g.Dias || g.dias || 'N/D';
                        return `
                        <div class="ficha-filter-row" onclick="app.filtrarPorAtributo('gasolina', 'mes', '${mes}', this)" 
                            ...
                            <div>
                                <span style="font-size:0.8rem; font-weight:600; color:#0f172a; text-transform:capitalize;">${mes}</span>
                                <span style="font-size:0.65rem; color:#64748b;"> | ${litros}L × ${dias}d</span>
                            </div>
