export default class ComponentePOE {
    constructor(appController) {
        ...
        // VALORES FIJOS
        this.TOTAL_POE = 103;            // 1 por cara
        this.COSTO_TOTAL = 113300;       // $113,300
        this.COSTO_UNITARIO = 1100;      // $1,100 c/u
    }

    getNombre() { return 'POE Utepo'; }

    // ✅ Calcular costo de licencia OPEX total
    getCostoLicenciaTotal() {
        const licPOEAnual = 300; // Licencia anual por POE (ajusta según tu OPEX_CONFIG)
        const factorMeses = this.app.state.meses.includes('all') ? 1 : (this.app.state.meses.length / 12);
        return (licPOEAnual * this.TOTAL_POE * factorMeses);
    }

    // ✅ Calcular costo de licencia OPEX por unidad
    getCostoLicenciaPorUnidad() {
        const licPOEAnual = 300;
        const factorMeses = this.app.state.meses.includes('all') ? 1 : (this.app.state.meses.length / 12);
        return (licPOEAnual * factorMeses);
    }

    // ✅ Calcular costo de licencia OPEX por ubicación (1 POE por cara)
        ...
        pantallas.forEach(p => {
            ...
            Object.values(dt).forEach(cara => {
                if (cara.poe && cara.poe.trim()) {
                    const modelo = cara.poe.trim();
                    modelos[modelo] = (modelos[modelo] || 0) + 1;
                    
                    const estado = (cara.poeEstado || '').toLowerCase().trim();
    ...
    renderFicha(totales) {
        const modelosHTML = Object.keys(totales.modelos).length > 0 
            ? Object.keys(totales.modelos).map(m => `
                <div class="ficha-filter-row" 
                     onclick="app.filtrarPorAtributo('poe', 'poe', '${m}', this)" 
                    ...
                    <div>
                        ...
                        <div style="font-size:0.75rem; color:#64748b;">${totales.modelos[m]} ${totales.modelos[m] === 1 ? 'POE' : 'POEs'}</div>
                    </div>
        ...
        <div style="font-size: 1.6rem; font-weight: 900; color: #0f172a; margin-bottom: 25px; letter-spacing: -0.5px;">POE UTEPO</div>
        
        <div style="display:flex; gap:12px; margin-bottom:25px;">
            <div class="ficha-filter-card" onclick="app.filtrarPorAtributo('poe', 'global', 'todas', this)"
            ...
            <div class="ficha-filter-card" onclick="app.filtrarPorAtributo('poe', 'poeEstado', 'Vulnerable', this)"
                 style="flex:1; border:1px solid #e2e8f0; border-radius:12px; padding:18px 10px; text-align:center; background:#fff; box-shadow:0 1px 2px rgba(0,0,0,0.02); cursor:pointer; transition:all 0.2s ease;"
                 onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 6px -1px rgba(0,0,0,0.1)';" 
                 onmouseout="this.style.transform='none'; this.style.boxShadow='0 1px 2px rgba(0,0,0,0.02)';">
                <div style="font-size:0.65rem; color:#64748b; font-weight:800; display:flex; flex-direction:column; align-items:center; gap:6px; text-transform:uppercase;">
        ...
        <div style="border:1px solid #e2e8f0; border-radius:12px; padding:15px; margin-bottom:15px; background:#fff; box-shadow:0 1px 2px rgba(0,0,0,0.02);">
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; padding:14px 12px; border-bottom:1px solid #f1f5f9;">
                <div>
                    ...
                    <div style="font-size:0.75rem; color:#64748b;">1 POE por cara</div>
                </div>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; padding:14px 12px; border-bottom:1px solid #f1f5f9;">
                <div>
                    ...
                    <div style="font-size:0.75rem; color:#64748b;">UTEPO</div>
                </div>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; padding:14px 12px; border-bottom:1px solid #f1f5f9;">
                <div>
                    ...
                    <div style="font-size:0.75rem; color:#64748b;">Costo anual por POE</div>
                </div>
    ...
    renderDetalleUbicacion(pantalla) {
        ...
        return caras.map(cara => {
            const c = dt[cara];
            const modelo = c.poe || 'N/D';
            const estado = c.poeEstado || 'N/D';
