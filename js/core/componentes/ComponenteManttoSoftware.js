export default class ComponenteManttoSoftware {
    constructor(appController) {
        ...
        this._interval = setInterval(() => {
            ...
            if (mesesActuales !== this._mesesAnteriores) {
                ...
                if (this.app.state.componente === 'software') {
                    this.app.generarFichaDinamicaNativa('software', 2);
                }
            }
        }, 500);
    }

    getNombre() { return 'Mantenimiento Software'; }

    _esSoftware(actividad) {
    ...
    _calcularCostoPantalla(pantalla, mesesActivos) {
        ...
        for (const ticket of tickets) {
            ...
            if ((mesesActivos.includes('all') || mesesActivos.includes(mes)) &&
                this._esSoftware(ticket.actividad || '')) {
    ...
    renderFicha(totales) {
        ...
        return `
            ...
            <div style="font-size:1.6rem; font-weight:900; color:#0f172a; margin-bottom:25px;">MANTTO. SOFTWARE</div>
            <div style="display:flex; gap:12px; margin-bottom:25px;">
                <div class="ficha-filter-card" onclick="app.filtrarPorAtributo('software', 'global', 'todas', this)" style="flex:1; border:1px solid #e2e8f0; border-radius:12px; padding:18px 10px; text-align:center; background:#fff; cursor:pointer;">
    ...
    renderDetalleUbicacion(pantalla) {
        ...
        const filtrados = tickets.filter(t => {
            ...
            return this._esSoftware(t.actividad || '') && 
        ...
        return `
            <div style="background:#fff; border:1px solid #e2e8f0; border-radius:10px; padding:15px;">
                <div style="display:flex; justify-content:space-between; border-bottom:1px solid #f1f5f9; padding-bottom:10px; margin-bottom:12px;">
                    <span style="font-weight:900;">Software - ${pantalla.id}</span>
                ...
                ${filtrados.map(t => `
                    <div style="display:flex; justify-content:space-between; padding:8px 10px; background:#f8fafc; border-radius:6px; margin-bottom:4px;">
                        ...
                        <span style="font-weight:900; color:var(--theme-color);">${this.formatMoney(Number(t.costoManttoOriginal || 0))}</span>
                    </div>`).join('')}
            </div>`;
    }
}